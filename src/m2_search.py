        ...
        for rank, result in enumerate(result_list):
            if result.text not in rrf_scores:
                ...
                for mk, mv in result.metadata.items():
                    if mk not in rrf_scores[result.text]["result"].metadata:
                        rrf_scores[result.text]["result"].metadata[mk] = mv
            rrf_scores[result.text]["score"] += 1.0 / (k + rank + 1)

    sorted_results = sorted(rrf_scores.values(), key=lambda item: item["score"], reverse=True)
    return [
        SearchResult(
